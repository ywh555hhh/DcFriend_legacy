...
async def main():
    ...
    # 步骤 1: 创建依赖注入容器实例。
    # 根据官方文档，只要在 Container 类中定义了 wiring_config，
    # 这一步就会自动触发 .wire() 操作，为后续的依赖注入做好准备。
    logger.info("创建依赖注入容器...")
    container = Container()
    
    # 步骤 2: 从我们的配置系统加载配置到容器中。
    # 这会将 .env 文件中的值（通过 settings 对象）填充到容器的配置提供者中。
    # 注意：我们不再需要手动加载 .env，因为 `src.core.config.settings` 已经完成了。
    # 这个版本已在 container.py 内部处理，这里保持为空，或者可以移除。
    # 如果你的 container.py 依赖 config provider, 像这样：
    # container.config.from_pydantic(settings)

    # 步骤 3: 定义并创建机器人实例。
    logger.info("定义机器人意图 (Intents)...")
    intents = discord.Intents.default()
    intents.message_content = True  # 订阅消息内容事件
    intents.members = True          # 订阅服务器成员事件
    ...
    # 这使得在项目的任何地方（特别是测试中）都可以通过 `bot.container` 方便地访问到容器。
